...
class CreditCardValidator
  def determine_card_type(card_digits_str)
    ...
    if card_digits_str =~ /^3[47]\d{13}$/
      "AMEX"
    elsif card_digits_str =~ /^6011\d{12}$/
      "Discover"
    elsif card_digits_str =~ /^5[1-5]\d{14}$/ 
      "MasterCard"
    elsif card_digits_str =~ /^4[0-9]{12}(?:[0-9]{3})?$/
      "VISA"
    else
      "Unknown"
    end
  ...
  def validate_card_luhn(card_digits_str)
    begin
      # Make sure the input is an integer, otherwise fail
      Integer(card_digits_str)
